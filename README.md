...
- [ ] **디바운스 (Debounce) & 쓰로틀 (Throttle)**
  - [ ] 디바운스 개념과 원리
    - 이벤트가 연속으로 발생할 때 마지막 이벤트 후 일정 시간이 지난 뒤에만 실행
    - `clearTimeout` / `setTimeout`을 활용한 타이머 재설정 방식
    - 검색창 입력처럼 글자마다 API 요청이 발생하면 서버 부하가 커지는 문제 해결
  - [ ] 쓰로틀 개념과 원리
    - 일정 시간 간격으로 최대 1번만 실행 (스크롤 이벤트 등에 활용)
    - 디바운스와의 차이점: 디바운스는 마지막 이벤트 기준, 쓰로틀은 주기 기준
  - [ ] 실무 활용 사례
    - 검색창: 디바운스 300~500ms 적용으로 불필요한 API 요청 최소화
    - 무한 스크롤: 쓰로틀 적용으로 스크롤 이벤트 횟수 제한
    - 윈도우 리사이즈 감지: 쓰로틀 적용
  - [ ] AbortController를 활용한 요청 취소
    - 새 요청이 시작될 때 이전 진행 중인 요청을 네트워크 레벨에서 강제 취소
    - 느린 네트워크에서 이전 응답이 늦게 도착해 결과가 뒤바뀌는 문제(Race Condition) 방지
    - 디바운스와 함께 사용하면 요청 횟수 감소 + 응답 순서 보장까지 가능
